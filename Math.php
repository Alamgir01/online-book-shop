		...
		$start=($curentpage - 1)*$perpageres;
		   ...
		   if($curentpage <=0){
			...
			$sql = "SELECT books.Book_id as id,books.Book_title as Title,
                     ...
                     where category.Name = \"Math\"  LIMIT $start,$perpageres; ";
		     ...
		     <div id="cse">      
			  ...
			  <div id="link" style="text-align:center;">
                ...
                <?php 
 				...
 				$sql1 = "select Book_id from books where category_id = 5;";
					   ...
					   $total_pages = ceil($rows/$perpageres);	
						...
						if($curentpage>1){
							echo "<a href='Math.php?page=".$prev."'> Prev </a>";
						}
				        $pagLink = "";						 
							for ($i=1; $i<=$total_pages; $i++) { 
									if ($i==$curentpage) { 
										$pagLink .= "<a href='Math.php?page=".$i."' class='active'>".$i."</a>"; 
									}			 
									else { 
										$pagLink .= "<a href='Math.php?page=".$i."'> ".$i."</a>"; 
									} 
							}
							echo $pagLink;
                            if($curentpage<$total_pages){	
							echo "<a href='Math.php?page=".$next."'> Next </a>";
						}									 
				?>			  
			  </div>
		</div>
