		...
		$start=($curentpage - 1)*$perpageres;
		   ...
		   if($curentpage <=0){
			...
			$sql = "SELECT books.Book_id as id,books.Book_title as Title,
                     ...
                     where category.Name = \"Civil\"  LIMIT $start,$perpageres; ";
...
<body onload="actv()">
		...
		<div id="link" style="text-align:center;">
					...
					$sql1 = "select Book_id from books where category_id = 4;";
						   ...
						   $total_pages = ceil($rows/$perpageres);	
							...
							if($curentpage>1){
								echo "<a href='Civil.php?page=".$prev."'> Prev </a>";
							}
							$pagLink = "";						 
								for ($i=1; $i<=$total_pages; $i++) { 
										if ($i==$curentpage) { 
											$pagLink .= "<a href='Civil.php?page=".$i."' class='active'>".$i."</a>"; 
										}			 
										else { 
											$pagLink .= "<a href='Civil.php?page=".$i."'> ".$i."</a>"; 
										} 
								}
								echo $pagLink;
								if($curentpage<$total_pages){	
								echo "<a href='Civil.php?page=".$next."'> Next </a>";
							}									 
					?>			  
			  </div>
